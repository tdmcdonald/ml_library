...
features=all_features[0:2400,:]

pred_features= features
# features=np.transpose(features)
train_labels= np.asarray(lable)[0:24000]
testfeatures=all_features[24000:30000,:]
test_labels=np.asarray(lable)[24000:30000]
print(features)
...
for itr in np.arange(1,1000):
    ...
    # if True:
        bagged = Ensemble.AdaBoost(itr)
        bagged.fit(features,new_labels)
        ...
        for i,p in enumerate(predictions_train):
            # print(p, new_labels[i])
            if p!=new_labels[i]:
                # print('prediction is ', p, 'actual is ', new_labels[i])
                error_num+=1
                # print('error at ', i)
        iter_counts.append(itr)
        print('train err ',(error_num/len(new_labels)))

        train_errors.append(error_num/len(new_labels))
        error_num=0
        for k,p in enumerate(predictions_test):
            ...
            if p!=new_test_labels[k]:
                ...
                error_num+=1
                # print('error at ', i)
        print('test err ',(error_num/len(new_test_labels)))
        test_errors.append(error_num/len(new_test_labels))
        plt.clf()
        plt.cla()       
