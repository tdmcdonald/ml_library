...
class DecisionTree:
    def __init__(self, criterion, max_depth=math.inf, random=False, sample_num=2):
    ...
    def bestSplit(self, A, labels, weights): #A will have the same dims as self.features
        ...
        if self.criterion=='entropy':
            ...
            tot=0
            # print('WEIGHTS ARE ',weights)
            for i,label in enumerate(labels): #this increments the lable counts to find global entropy
                if label not in label_counts:
                    label_counts[label]= 0
                label_counts[label]+=1 *weights[i]
                tot+=1 *weights[i]
            # print('LABEL COUNTS ARE',label_counts)
            # tot=len(labels)
            current_entropy= 0.0
            ...
            for k,l in enumerate(A): #for every attribute
                ...
                for i,v in enumerate(A[k]):      #for every sample in attribute accumulate total outcomes
                    if v not in feature_outcome_counts[k]:
                        # feature_counts[k][v]={}
                        feature_outcome_counts[k][v]=0
                    feature_outcome_counts[k][v]+=1 * weights[i]
                ...
                for i,sample in enumerate(A[attribute]): #loop through samples to accumulate values
                    ...
                    for label in self.labels:
                        ...
                        if labels[i]==label:
                            ...
                            feature_counts[attribute][sample][label]+=1 * weights[i]
            ...
            # for attr in feature_entropy:
                # # print('for attr',attr)
                # print('feature counts is ', feature_counts[attr])
                # print('outcomes is', feature_outcome_counts[attr], 'feat counts is ', tot)
                # print('entropy is', feature_entropy[attr], 'feat counts is ', tot)
           ...
           #Feat outcome count is indexed by value
            ...
            max_ig=0
            max_ig_attr=None 
            # print('feat entropy is ', feature_entropy, 'expt is ', expected_entropy)
            for attribute in expected_entropy:
                ig[attribute]=current_entropy-expected_entropy[attribute]
                # print('for attribute ',attribute,'ig is ', ig[attribute])
...
#########################################################################
    ...
    def id3(self, S, attributes, labels,  weights,depth=0):
        # print('id3', weights)
        first=labels[0]
        switch=0
        ...
        if len(attributes)==0:
            ...
            if self.random:
                ...
                sampled=np.random.choice(indices, self.sample_num)
                
                for i, ind in enumerate(indices):
                    if ind in sampled:
                        new_feats.append(True)
                    else:
                        new_feats.append(False)
                
                samples=samples[new_feats]
                # print('new feats is', new_feats)
                # print('as array', np.asarray(new_feats))
                a = self.bestSplit(samples, labels, weights)
            else:
                a = self.bestSplit(S, labels, weights)
            ...
            if depth ==self.max_depth:
                ...
                for v in self.features[best_attr]:
                    ...
                    leaf= Node()
                    leaf.type ='leaf'
                    leaf.label= self.getMaxLabelofFeature(labels,v, S, a,weights) 
                    # print('setting label to ',leaf.label)
            ...
            for v in self.features[best_attr]: # want sv to be S where A=v v will be all values a can have & create branch NOTE: s.features isa  set
                ...
                if(count==0):
                    ...
                    # print('APPENDING NEXT TO ATTRIBUTE' , best_attr, temp_attributes)
                    nxt_root= self.id3(Sv,temp_attributes,temp_labels, temp_weights,depth)
                    # nxt_root.value=v
    ...
    def getMaxLabel(self,labels):
        ...
        return  m
    def getMaxLabelofFeature(self,labels,v, S, a, weights ):
        # print('HERERERERERERERER')
        # print('feature is ', v, 'len of lables is ', len(labels), 'len of sa is ',len(S[a]))
        counts={}
        ...
        for i,x in enumerate(S[a]):
            ...
            if S[a][i] == v:
                if labels[i] not in counts:
                    counts[labels[i]]=0
                counts[labels[i]]+=1*weights[i]
        if len(counts)>0:
            m=max(counts, key=counts.get)
        else:
            m=self.getMaxLabel(labels)
        # print('labels', labels,'max',m, 'counts',counts)
        return  m
    ...
    def fit(self, training_feats, training_labels,  numerical_cols=[],unknown_cols=[], weights=[] ):
        # print('fit',training_feats)
        # first looop through features to define self.features
        if(len(weights)==0):
            weights=np.ones(len(training_labels),dtype=int)
        training_features=copy.deepcopy(training_feats)
        ...
        self.root = self.id3(training_features, self.attributes, training_labels,weights)
